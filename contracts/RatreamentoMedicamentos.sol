...
contract RastreamentoMedicamentos {
    ...
    function cadastrarAdministrador(
        ...
        administradores[_adminAddress] = Administrador({
            ...
            ativo: false // Inicialmente desativado
        });

        emit AdministradorCadastrado(_adminAddress, _nomeCompleto);
    }

    // Função para ativar o administrador
    function ativarAdministrador(address _adminAddress) public somenteProprietario {
        require(administradores[_adminAddress].ativo == false, "Administrador ja esta ativo.");
        administradores[_adminAddress].ativo = true;
    }
